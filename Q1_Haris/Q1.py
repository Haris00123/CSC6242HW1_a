...
class Graph:
    ...
    def add_node(self, id: str, name: str) -> None:
        ...
        """
        'Creating the tuple to add into the nodes parameter'
        to_add=(id,name)

        'Simple if statement to check to see if the node exists, if it does not, add it. If it does, do nothing'
        if to_add in self.nodes:
            return None
        else:
    ...
    def add_edge(self, source: str, target: str) -> None:
        ...
        to_add=(source,target)
        to_add_rev=(target,source)

        'Simple if statement to check to see if the edge exists, if it does not, add it. If it does, do nothing'
        if to_add in self.edges or to_add_rev in self.edges:
            return None
        else:
    ...
    def max_degree_nodes(self) -> dict:
        ...
        for edge in self.edges:
            one_d_edges.append(edge[0])
            one_d_edges.append(edge[1])
        counts=Counter(one_d_edges)
        mx_dg=counts.most_common(1)[0]
        return {mx_dg[0]:mx_dg[1]}
...
# - You should use the param 'language=en-US' in all API calls to avoid encoding issues when writing data to file. - Done
# - If the actor name has a comma char ',' it should be removed to prevent extra columns from being inserted into the .csv file - 
# - Some movie_credits do not return cast data. Handle this situation by skipping these instances. - Added None return in functon, need to add in final go around
...
#   - wait a while and then try again.  It may be necessary to insert periodic sleeps when you are building your graph. - Not needed
...
def deal_loop(graph:Graph,tmdb_api_utils:TMDBAPIUtils,cutoff:float=8.0,limit:int=3\
    ,initial_run:bool=False,initial_id:str=None,initial_name:str=None)->None:
    '''Function to deal with the loops in the similar movie calls
    :param Graph graph: The graph instance
    :param TMDBAPIUtils tmdb_api_utils: TMDB movie API caller instance
    :param float cutoff: The average movie score cutoff for the movie selection call
    :param int limit: The limit order number for the cast function call
    :param bool initial_run: Identifies if this is the first run of the function
    :param str initial_id: The initial id of the seed
    '''
    ...
    if initial_run:
        ...
        nodes=graph.get_nodes().copy()
        node_og=(initial_id,initial_name)
        nodes.remove(node_og)
        loops=2
    ...
    for i in range(loops):
        ...
        for node in nodes:
            node_id=node[0]
            similar_movies=tmdb_api_utils.get_movie_credits_for_person(node_id,cutoff)

            if similar_movies is None:
                continue

            for movie in similar_movies:
                cast_members=tmdb_api_utils.get_movie_cast(movie['id'],limit=limit,exclude_ids=[node_id])

                if cast_members is None or len(cast_members)==0:
                    pass
                else:
...
if __name__ == "__main__":
    ...
    tmdb_api_utils = TMDBAPIUtils(api_key='dda851cd3e59527d775becc901149227')

    print('\n')
    print('Proceeding to first loop')

    deal_loop(graph,tmdb_api_utils,initial_run=True,initial_id='2975',initial_name='Laurence Fishburne')
    print('\n')
    print('First loop done')
    print('\n')
    print('Proceeding to second loop')
    deal_loop(graph,tmdb_api_utils,initial_run=False,initial_id='2975',initial_name='Laurence Fishburne')
